<!-- agentic-artifact:
  schema: agentic-artifact/v2
  id: education.workflows.readme
  version: 1
  status: active
  layer: 05.education
  domain: education
  disciplines:
  - agentic
  kind: workflow
  purpose: Document Education Workflows.
  portability:
    class: required
    targets:
    - llm-workbench
  used_by:
  - id: education.readme
    path: .agentic/education/README.md
-->
# Education Workflows

## Workflows

- `mine-daily-learning-material.md` - mine commit logs and repo evidence for
  short public article, talk, classroom, and teaching-asset opportunities.
- `create-educational-resource.md` - draft a selected educational resource from
  mined material and profiles.
- `calibrate-education-outputs.md` - turn feedback on generated resources into
  profile, template, or prompt updates.
- `improve-education-system.md` - distill source-backed teaching, writing,
  humor, talk-design, and storytelling principles into education artifacts.
