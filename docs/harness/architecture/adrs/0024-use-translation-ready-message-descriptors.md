<!-- agentic-artifact:
schema: agentic-artifact/v2
id: harness.architecture.adr.0024-use-translation-ready-message-descriptors.pointer
version: 1
status: active
layer: 01.harness
domain: architecture
disciplines:
- architecture
kind: compatibility-pointer
purpose: Point older ADR 0024 references to the product-layer canonical ADR.
portability:
  class: source-only
  targets: []
used_by:
- id: rag-rulebook.migration-plan.prototype-corpus-domain-split
  path: .agentic/02.rag-rulebook/plans/migration/prototype-corpus-domain-split.md
-->
# ADR 0024 Moved

The canonical ADR now lives at
`docs/03.product/adrs/0024-use-translation-ready-message-descriptors.md`.

This compatibility pointer exists while the prototype architecture ADR corpus
is split into owner-aligned ADR roots.
