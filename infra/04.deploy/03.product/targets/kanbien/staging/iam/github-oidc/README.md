...
| `github-platform-shell-staging-reconciliation-policy.json` and `github-platform-shell-staging-reconciliation-trust.json` | Passively reconcile declared staging deployment controls. | Exact CloudFormation stack-summary reads, artifact-bucket control reads, one budget read, and caller identity only. | GitHub Actions for this repository's `main` branch only; it cannot start drift scans, deploy, alter stacks, read secrets, or operate ECS/RDS. |
