...
| `github-platform-shell-staging-reconciliation-policy.json`, `github-platform-shell-staging-reconciliation-trust.json`, and `reconciliation-operation-authorization-contract.yml` | Passively reconcile declared staging deployment controls. | Exact CloudFormation stack-summary reads, artifact-bucket control reads, one budget read, and caller identity only. The contract maps each CLI operation to its authoritative IAM action and target scope. | GitHub Actions for this repository's `main` branch only; it cannot start drift scans, deploy, alter stacks, read secrets, or operate ECS/RDS. |
