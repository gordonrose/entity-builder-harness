...
IAM_REMEDIATION_FRESH_REQUEST_ID = "persistence-smoke-iam-remediation-v4"
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--approve-fresh-after-iam-remediation",
        action="store_true",
        help="Acknowledge the one fresh write permitted only after the recorded least-privilege IAM remediation proof.",
    )
    ...
    if arguments.validate and arguments.approve_fresh_after_iam_remediation:
        parser.error("--approve-fresh-after-iam-remediation is valid only with --execute")
    if sum((
        arguments.approve_replacement_after_remediation,
        arguments.approve_fresh_after_admission_probe,
        arguments.approve_fresh_after_iam_remediation,
    )) > 1:
...
def resolve_policy(profile: dict[str, Any]) -> dict[str, str]:
    ...
    allowed_statuses = {
        ...
        IAM_REMEDIATION_SOURCE_READY,
        IAM_REMEDIATION_DEPLOYED_READY,
...
def main() -> int:
    ...
    if arguments.validate:
        emit("validated")
        return 0
    if arguments.approve_fresh_after_iam_remediation:
        if policy["status"] != IAM_REMEDIATION_DEPLOYED_READY:
            raise PersistenceSmokeError("the IAM-remediated fresh acceptance proof may run only after its recorded least-privilege authorization proof")
        request_id = IAM_REMEDIATION_FRESH_REQUEST_ID
    elif arguments.approve_fresh_after_admission_probe:
