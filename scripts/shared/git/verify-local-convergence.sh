...
usage() {
  cat <<'EOF'
Usage:
  verify-local-convergence.sh [--base <branch>] <chat-branch>

Read-only preflight for promoting a completed chat branch into local main.
Classifies deterministic blocked states and exits non-zero unless the branch is
eligible for explicit, user-approved local convergence.
EOF
}
...
# shellcheck source=../chat/chat-worktree-paths.sh
...
if [ "$REPO_ROOT" != "$PRIMARY_PATH" ]; then
  block "blocked-not-root-integration-worktree" \
    ...
    "Run local convergence verification from the root integration worktree."
fi
