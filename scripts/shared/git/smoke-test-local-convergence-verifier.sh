...
# agentic-script:
#   owner: 00.chat
#   purpose: Smoke test local convergence verifier classifications.
#   domain: git
...
run_expect_state() {
  ...
  if [ "$should_succeed" = "yes" ]; then
    (
      cd "$repo"
      bash scripts/shared/git/verify-local-convergence.sh "$branch"
    ) > "$out" 2> "$err" || {
      cat "$out" >&2
      cat "$err" >&2
...
echo "local convergence verifier smoke test passed."
