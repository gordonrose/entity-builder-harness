...
function assertInsidePackage(specifier, resolvedPath, relative) {
  ...
  assert.equal(
    ...
    false,
    `${relative} relative import must stay inside platform/server, found ${specifier}`,
  );
}
...
assert.equal(
  ...
  undefined,
  "platform/server production dependencies must not include platform/testing",
);

const sourceFiles = (await walk(srcRoot)).filter((file) => file.endsWith(".ts"));
assert.ok(sourceFiles.length > 0, "platform/server should expose source files");
  ...
  for (const pattern of importSpecifierPatterns) {
    for (const match of text.matchAll(pattern)) {
      ...
      assert.ok(
        ...
        `${relative} may only import Node HTTP primitives, platform runtime/contracts, or core contracts, found ${specifier}`,
      );
    }
  }
...
console.log(`platform/server boundary check passed for ${sourceFiles.length} source file(s).`);
