...
const allowedSourceImportPattern = /^@kanbien\/(?:core(?:\/[a-z][a-z0-9-]*)?|platform-contracts|platform-observability)$/;
...
function assertInsidePackage(specifier, resolvedPath, relative) {
  ...
  assert.equal(
    ...
    false,
    `${relative} relative import must stay inside platform/health, found ${specifier}`,
  );
}

const packageJson = JSON.parse(await readFile(packageJsonPath, "utf8"));
assert.equal(packageJson.name, "@kanbien/platform-health");
assert.equal(packageJson.exports?.["."], "./src/index.ts");
assert.equal(packageJson.dependencies?.["@kanbien/platform-observability"], "0.0.0");

const sourceFiles = (await walk(srcRoot)).filter((file) => file.endsWith(".ts"));
assert.ok(sourceFiles.length > 0, "platform/health should expose source files");
...
console.log(`platform/health boundary check passed for ${sourceFiles.length} source file(s).`);
