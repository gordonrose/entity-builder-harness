...
const forbiddenProviderWords = /\b(?:aws|amazon|dynamodb|sqs|eventbridge|cloudwatch|prisma|postgres|mysql|redis|kafka|rabbitmq)\b/i;
...
function assertInsidePackage(specifier, resolvedPath, relative) {
  ...
  assert.equal(
    ...
    false,
    `${relative} relative import must stay inside platform/persistence, found ${specifier}`,
  );
}

const packageJson = JSON.parse(await readFile(packageJsonPath, "utf8"));
assert.equal(packageJson.name, "@kanbien/platform-persistence");
...
assert.ok(sourceFiles.length > 0, "platform/persistence should expose source files");
...
console.log(`platform/persistence boundary check passed for ${sourceFiles.length} source file(s).`);
