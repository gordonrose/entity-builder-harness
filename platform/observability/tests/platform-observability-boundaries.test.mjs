...
function assertInsidePackage(specifier, resolvedPath, relative) {
  ...
  assert.equal(
    ...
    false,
    `${relative} relative import must stay inside platform/observability, found ${specifier}`,
  );
}
...
console.log(`platform/observability boundary check passed for ${sourceFiles.length} source file(s).`);
