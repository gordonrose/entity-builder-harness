...
const runtimeRoot = ".cache/platform-adapter-aws-auth-cognito-runtime";
const testDirectory = join(runtimeRoot, "platform/adapters/aws/auth/cognito/tests");
const coreModules = [
  "audit",
  "authn",
  "authz",
...
for (const packageName of [
  ...
  writePackageShim(`@kanbien/${packageName}`, {
    ".": join(runtimeRoot, `platform/${packageName.replace("platform-", "")}/src/index.js`),
  });
}
...
if (testFiles.length === 0) {
  throw new Error(`No Cognito adapter runtime tests found in ${testDirectory}.`);
}
