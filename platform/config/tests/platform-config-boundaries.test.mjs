...
function assertInsidePackage(specifier, resolvedPath, relative) {
  ...
  assert.equal(
    ...
    false,
    `${relative} relative import must stay inside platform/config, found ${specifier}`,
  );
}

const packageJson = JSON.parse(await readFile(packageJsonPath, "utf8"));
assert.equal(packageJson.name, "@kanbien/platform-config");
...
assert.ok(sourceFiles.length > 0, "platform/config should expose source files");
...
console.log(`platform/config boundary check passed for ${sourceFiles.length} source file(s).`);
